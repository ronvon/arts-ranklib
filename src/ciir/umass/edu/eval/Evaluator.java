...
public class Evaluator {
	...
	public static void main(String[] args) {
		...
		if(args.length < 2)
		{
            ...
            System.out.println("\t[ -boost <t> ]\t\tBoost Level to Apply Validate Judge (default=" + LambdaMART.boostJudge + ")");
			...
			//System.out.println("\t[ -sparse ]\t\tUse sparse representation for all feature vectors (default=dense)");
            System.out.println("\t[ -res ]\t\tResults file to append!");
		...
		for(int i=0;i<args.length;i++)
		{
			if(args[i].compareTo("-train")==0)
				trainFile = args[++i];
            else if(args[i].compareTo("-res")==0)
                resFile = args[++i];
			...
			} else if (args[i].compareTo("-boost") == 0) {
                LambdaMART.boostJudge = Integer.parseInt(args[++i]);
            }
		...
		if(trainFile.compareTo("")!=0)
		{
			...
			else
			{
				if(ttSplit > 0.0)//we should use a held-out portion of the training data for testing?
					...
					e.evaluate(trainFile, validationFile, testFile, featureDescriptionFile, resFile);//All files except for trainFile can be empty. This will be handled appropriately
			}
		}
    ...
    public float computeModelScoreOnOriginal(List<RankList> pSamples)
    {
        float s = 0;

        for(int i = 0; i < pSamples.size(); i++) {
            s += testScorer.score(pSamples.get(i));
        }
	 ...
	 */
	public void evaluate(String trainFile, String validationFile, String testFile, String featureDefFile, String resFile)
	{
        //todo read input
        ...
        String params = "";
        if (resFile.compareTo("") != 0) {
            params += "|-leaf:" + LambdaMART.nTreeLeaves;
            params += "|-mls:" + LambdaMART.minLeafSupport;
            params += "|-epoch:" + LambdaMART.nRoundToStopEarly;
            params += "|-shrinkage:" + LambdaMART.learningRate;
		...
		Ranker ranker = trainer.train(type, train, validation, features, trainScorer, fileWriter);
        ...
        if(normalize && test != null) { //never used by me
            normalize(test, features);
        }

		if(test != null) {
			double rankScore = evaluate(ranker, test);
            double rankScoreOri = computeModelScoreOnOriginal(test);
			System.out.println(testScorer.name() + " on test data: " + SimpleMath.round(rankScore, 6));

            double up = (rankScore - rankScoreOri) * 100 / rankScoreOri;
            BigDecimal bigDecimal = new BigDecimal(up);
            params = "|testOri:" + SimpleMath.round(rankScoreOri, 6);
            params += "|test:" + SimpleMath.round(rankScore, 6);
            params += "(" + bigDecimal.setScale(2, BigDecimal.ROUND_HALF_UP) + "%)";
		}
        if (fileWriter != null) {
            try {
                fileWriter.write(params + "\n");
