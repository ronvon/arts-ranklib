...
public class FeatureHistogram {
	...
	protected void update(double[] labels, int start, int end) //labels是所有sample的delta,即二阶偏导
	{
		for(int f=start;f<=end;f++) //编号为start的属性开始，到end结束
			Arrays.fill(sum[f], 0);
		for(int k=0;k<labels.length;k++) //k索引sample
		{
			for(int f=start;f<=end;f++) //f索引属性
			{
				int t = sampleToThresholdMap[f][k]; //找出k-sample在f-feature下归到哪个split区间了
				sum[f][t] += labels[k]; //XGM(y)
				if(f == 0)
				{
					sumResponse += labels[k];  //所有的y的和
					sqSumResponse += labels[k]*labels[k]; //所有的y^2的和
				}
		...
		{			
			for(int t=1;t<thresholds[f].length;t++)
				sum[f][t] += sum[f][t-1]; //feature-f左边的y(lambda)之和
		}
	}
	...
	protected Config findBestSplit(int[] usedFeatures, int minLeafSupport, int start, int end)
	{
		...
		int totalCount = count[start][count[start].length-1]; //只是这么计算samples的总数totalCount
		for(int f=start;f<=end;f++)
		{
			...
			for(int t=0;t<threshold.length;t++)
			{
				...
				double S = sumLeft * sumLeft / countLeft + sumRight * sumRight / countRight; //使得S最大
				if(cfg.S < S) //cfg.S初始化为负数,而S为正数
				{
					cfg.S = S;
					cfg.featureIdx = i;
					cfg.thresholdIdx = t;
	...
	public boolean findBestSplit(Split sp, double[] labels, int minLeafSupport)
	{
		...
		double[] sumLabel = sum[best.featureIdx]; //best.featureIdx分隔的feature的IDX,best.thresholdIdx分隔的threshold的idx
		...
		double sumRight = s - sumLeft; //左右边的sample的模型打分之和
		...
		double var = sqSumResponse - sumResponse * sumResponse / idx.length;//mean squared error "S"
