...
public class RankerTrainer {
    ...
    public Ranker train(RANKER_TYPE type, List<RankList> train, List<RankList> validation, int[] features, MetricScorer scorer, FileWriter fileWriter)
    {
        Ranker ranker = rf.createRanker(type, train, features, scorer);
        ranker.setValidationSet(validation);
        ranker.setFileWrite(fileWriter);
        long start = System.nanoTime();
        ranker.init();
        ranker.learn();
        trainingTime = System.nanoTime() - start;
